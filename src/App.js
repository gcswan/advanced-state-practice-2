...
class App extends Component {
  ...
  render() {
    ...
    const makes = vehicleData.map((vehicle) => {
      return (
        <option value={vehicle.value}>{vehicle.title}</option>
      )
    })
    

   const filteredCars = carsForSale.filter((car) => {
     if (car.year == this.state.currentYear || car.make == this.state.currentMake){
       return true;
     };
   })
  ...
  const renderCarListing = filteredCars.map((car) => {
    return (
      <CarListing car={car} />
    )
  })
                  ...
                  <div className="sidebar_filter">
                    <form action="http://themes.webmasterdriver.net/carforyou/demo/listing-grid.html#" method="get">
                      <div className="form-group select">
                        <select onChange={(e) => {if (e.target){this.setState({currentYear: e.target.value});}}} className="form-control">
                      ...
                      <div className="form-group select">
                        <select onChange={(e) => {if (e.target){this.setState({currentMake: e.target.value})}}} className="form-control">
